...
import { EVENT_ID_SERIAL_DIGITAL_UPDATE } from '../constants'; // Adjust the path as needed

// Create a Context for the data
export const SerialDigitalDataContext = createContext();

const SerialDigitalDataProvider = ({ children }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`${restUrl}/serial/digital/getall`); // Use the REST URL from context
      setData(response.data);
      console.log("Serial Data fetched:", response.data);
  ...
  useEffect(() => {
    messages.forEach((message) => {
      if (message.eventid === EVENT_ID_SERIAL_DIGITAL_UPDATE) {
  ...
  return (
    <SerialDigitalDataContext.Provider value={contextValue}>
      {children}
    </SerialDigitalDataContext.Provider>
  );
};

export default SerialDigitalDataProvider;
